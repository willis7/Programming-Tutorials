...
/*	Okay for starters what the heck is a queue?  Well a queue is nothing more than
	...
	"QUEUE" structure and some functions.
...
{
	int *array; // Pointer for our dynamic queue to be held in

	int max_elements; // This will hold our maximum capacity of integers that can
					  // be held by the queue
	
	int index; // This will always correspond to the next OPEN slot in the queue
			  // that means that the BACK of the queue will always be (index - 1)
...
// Remove() returns the FRONT element of the queue and removes it from the queue (ie everything
// gets moved down, so the element after the FRONT element is now the FRONT)
// If there are no elements in the queue, Remove() destorys your hard drive
...
/* Really that's all we need -- Now for safety sake it would probably be a good idea
   to change the int Remove(QUEUE *stack) to bool Remove(QUEUE *queue int *front);
