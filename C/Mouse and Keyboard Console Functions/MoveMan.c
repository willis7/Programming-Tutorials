...
CONSOLE_SCREEN_BUFFER_INFO ScrInfo;					// We declare our Screen Information structure.  We'll fill this with all the screen info for our 
													// console widnow.  We want this GLOBAL so all of our functions can utilize it.
...
{													// Notice that we put these variable inside of the function, because no one else uses them besides this funciton.
...
void main()											// We don't need to return anything from main.
{													// Start of Program.
