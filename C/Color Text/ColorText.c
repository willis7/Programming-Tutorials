...
void DrawColorString(char *szText, int X, int Y, WORD color)
{
	...
	HANDLE OutputH;										// This is a OUTPUT handle.  Think of this as our link to the video card.
	...
	length = strlen(szText);							// strlen() finds the amount of characters in a string.
														...
														// Know how bit the string is that is passed in, can't just create a colorBuffer like this:
	...
	colorBuf = NULL;									// After we free a pointer, we should always set it to NULL.  Checking if a pointer equals NULL is the way a pointer is determined if it's "valid" or not
														...
														// Or when you quit the program, your computer runs slower?  That's the not computer getting tired... that because the programmers
														// Didn't free their memory.  If you ask windows for memory, it locks that memory address so no other program can use it.
