...
int main() {
	...
	// But you may ask ... but now do I know what the error was if all I get is SOCKET_ERROR?  That is where
	...
	// To demonstrate WSAGetLastError() lets try something that will cause an error.
	...
	// For now we will not get into the specifics of TCP.  This will also come in a later tutorial.  An alternate type would
	// commonly be SOCK_DGRAM, which is a UDP socket.  UDP however, is not within the scope of these
	// tutorials, and may be covered at a later date.  We will primarily use TCP.
	...
	// WSAStartup().  It is appropriately named WSACleanup().  This function though requires
	// no arguments, and need only be called once.
	...
	// And congratulations!  You are well on your way to becoming a network programmer.
