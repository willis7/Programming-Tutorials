...
// our communication.  What we will do is set up one program to wait for someone to request a
...
// for networking computers, but it is the one most often used when creating networked games, so
...
// these functions will convert them back and forth between network ordering and the native machines
...
int main() {
	...
	// then discuss what is going on inside your computer.
	...
	// to the port we specify (7654).  So now when our client wants to send us data, he will send it
	...
	// for the third, and when a connection is accepted, the sockaddr struct would be filled with information
	// about the connecting client.  But since we are using TCP, all we NEED to communicate is the clientSocket.
	// So we pass in 0, 0 for both parameters, cause we don't really care about the information.
	...
	// it can begin accepting connections from that moment forward.  So, lets say we didn't IMMEDIATELY call
	// accept().  Lets even say you put it off for 10 or so minutes.  If a client tried to connect, his connection would
