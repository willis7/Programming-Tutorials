...
#include <iostream> // The #include (read "pound include") is used to include other
				...
				// (these would be header files that allow you to call C++ functions/methods
...
// can also be used for "char" (text) substitutions.  Again BEFORE this .cpp file is compiled
// the preprocessor will replace everywhere it see "TEXT" with "GameTutorials and
...
#else	// If the expression "#if NUM" is false, the code between the #else and #endif
		// will be compiled -- If the expression "#if NUM" is true NONE of the code
...
/*
  ...
  project -- This way you can have many .cpp files that #include the same header file
