...
void threadProc() {
	...
	// Remember to set our flag to true so our main thread will know we have started.1
	threadStarted = true;
...
void main() {
	...
	for (int i = 0; i < 10; i++) {
		...
		// we are done printing, let's release the mutex and allow threadProc to add more data to it.
		ReleaseMutex(mutexHandle);
	}
...
// same amount of time.  If both sleep values were 0, it may be hundreds or thousands of times of times before
...
// Since all the main thread is doing is printing what it sees in the buffer, it would be virtually impossible to cause a
// problem here.  But what if we WERE trying to manipulate the data in the buffer from our main thread?  This,
