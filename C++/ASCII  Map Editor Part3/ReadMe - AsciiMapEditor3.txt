...
Files:  	Main.cpp (The Source File containing the main program code)
		Tile.cpp (The Source File containing the tile and map code)
		Tile.h (The Header File containing the tile and map definitions)
		Main.h (The Header File containing the main program definitions)

		AsciiMapEditor.vcproj  (The VC++ Project File holding the project info)
		AsciiMapEditor.sln  (The Solution File holding the solution info)
		*.txt (These files store the data for the monster, item and npc tiles)
		Default.map (The example saved map file to test the loading)
...
Instructions:	If you have Microsoft Visual Studio .NET (version 7.0) just click on the
		...
		the Win32 console program should pop up. :)

		This version doesn't allow much functionality, but allows you to select
		different tiles from the editor and change their colors.  The next tutorial
		will show you how to insert those tiles on the map, along with other tiles.
