...
using namespace std; // As you can see, if you've been doing C++ coding chances are you've
				...
				// directly accessing them through the namespace"
...
// FLOAT_FUNCTIONS -- To put something inside of a namespace simply type:
/*
...
*/
namespace FLOAT_FUNCTIONS // The name of our namespace
{
	...
	int an_int; // Here we're declaring an int -- This is just here to show how we
			   // access it in main()
}
...
int main()
{
	cout.setf(ios::showpoint); // Whoa!  What the heck is this?
						  ...
						  // floating-point values (ie float, double)
	...
	// Notice to get to that particular version we first type the namespace (FLOAT_FUNCTIONS)
	// then the "scope resolution operator" (the double colons) -- Now we can access anything
	// that is contained within the scope (within the namespace) of FLOAT_FUNCTIONS so we can
	// call the "float add function"
	cout << endl << "Float add of 2 + 3 = " << FLOAT_FUNCTIONS::add(2, 3);
	
	cout << endl; // Print a return for fun :)

	// Now what if we wanted to get to the variable "an_int" ???

	// It's the same syntax:
	FLOAT_FUNCTIONS::an_int = 55; // We've set "an_int" to 55

	// Lets prove it by printing it out
	cout << "Integer = " << FLOAT_FUNCTIONS::an_int << endl;	
...
/*
  ...
  Namespaces are just yet another way C++ allows you to "label regions of code" so 
  hopefully NO functions or variables will ever "step on one another"
