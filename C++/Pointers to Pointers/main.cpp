/* Done by TheTutor 7/26/01 (Last Updated 9/6/04)

	All right what this tutorial is going to focus on is "pointers to pointers" also
	called "double pointers" -- Double pointers are really not to difficult of a concept
...
int main()
{
	...
	*/
	...
	// Now print out the contents of the matrix to the screen (so it looks something like
	// a matrix)
	for(i = 0; i < row; i++)
	{
		cout << endl;
	...
	// Now we must free the matrix -- First we're free all the "col" number of "integer arrays"
	// we allocated
	for(i = 0; i < row; i++)
		delete[] matrix[i];
