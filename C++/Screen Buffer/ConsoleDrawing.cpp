...
#include <windows.h>									// We need to include this for "console" functions.
														// Since we are not using cout, we don't need to include <iostream>
														// Or use a namespace.
...
void main()												// Here is our main().
{
	...
	for(y = 0; y < gridSize.Y; y++)						// This fills in the columns
	{
		for(int x = 0; x < gridSize.X; x++)				// This fills in the rows
		{
			...
			// Well, this is because the ASCII character 1 is for the smily face :)
...
//  and gridSize is the appropriate numbers, also your drawing position is set to (0, 0) (zerZero).
//
//	You will most likely get really frustrating trying to work with this stuff, so feel free
