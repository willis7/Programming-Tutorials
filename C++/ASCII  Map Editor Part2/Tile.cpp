...
void CMap::SetTileInfo(int type, char *szFile)
{
	...
	// and reads in all the data associated with each seperate tile type.  We have
	...
	while(!feof(fp))
	{
		if(type == ITEM_TYPE)					   // If the tile type is an item
		{
			...
			// in the life increase for the particlular item.  Then skips over "strengthInc:"
...
void CMap::InsertTile(CTile *pTile, int x, int y)
{
	...
	else									// Otherwise it must be a normal map tile
	{
		...
		// new tile, since we already did that in SetDefault() when we first 
...
void CMap::DeleteTile(int mapX, int mapY)
{
	...
	// current selected tile list and see if there is a tile with that same
