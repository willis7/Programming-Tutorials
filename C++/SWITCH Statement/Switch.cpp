...
{											// This starts the main function
	...
	// "if(number == 1)". We put a colen after the case.  Now, if the value of "number:
	...
	// we "break" out of this switch statement.  What the "break" does, it leave the
	...
	// unless there is a break. This is strange, but usefull, as seen in an example below this one.
	...
	{										// The beginning of the switch statements scope
		...
		default:							// If the number isn't 1, 2 or 3, complain :)
			...
			break;							// We don't need this really, but I like to for consistancy.
	...
	// Well, you will notice that if only checks for LOWER CASE characters.
	// We don't want to have to make a whole new case for lower and upper case characters,
	// so here is what we do: we just add another case right after the first one, seperated
	// by a colen (:).   We can do as many as we want.  We can also do it on each line, like this:
	// case 'a':
	// case 'A':
	//	cout etc...
