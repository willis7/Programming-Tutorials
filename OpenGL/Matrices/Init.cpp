...
{	
	...
	hWnd = CreateMyWindow("www.GameTutorials.com - Matrices", SCREEN_WIDTH, SCREEN_HEIGHT, 0, g_bFullScreen, hInstance);
...
// * QUICK NOTES * 
//
// We added Depth testing in our IntializeOpenGL() function.  This allows us to
// draw multiple shapes over each other without getting distance errors.  If we
// didn't depth test the primatives that were far away from the camera would draw
// over the closer ones.  We want to draw the closest ones to the screen last so this
// does not happen.	 This will probably be a standard thing from now on.
// 
